...
add_action( 'after_switch_theme', 'wtt_flush_rewrite_rules' );

// Flush your rewrite rules
function wtt_flush_rewrite_rules() {
	flush_rewrite_rules();
}


// Register Custom Post Type
function wtt_staff_cpt() {
	...
	register_post_type( 'wtt_staff', $args );

}
add_action( 'init', 'wtt_staff_cpt', 0 );


// Register Custom Taxonomy
function wtt_staff_grouping_tax() {
	...
	register_taxonomy( 'grouping', array( 'wtt_staff' ), $args );

}

add_action( 'init', 'wtt_staff_grouping_tax', 0 );


// Change the 'Title' field text on edit screen
function wtt_staff_change_title_text( $title ){
    $screen = get_current_screen();

    if  ( 'wtt_staff' == $screen->post_type ) {
...
add_filter( 'enter_title_here', 'wtt_staff_change_title_text' );


// Add sortable admin columns dopeness
add_filter("manage_edit-wtt_staff_sortable_columns", 'wtt_staff_sort');

function wtt_staff_sort($columns) {
    $custom = array(
